...
class VAE_encode(nn.Module):
    ...
    def forward(self, x, direction):
        ...
        if direction == "a2b":
            ...
            _vae = self.vae_b2a
            # Add this right after the VAE object is built/loaded
            _vae.enable_slicing()
            _vae.enable_tiling()
...
class VAE_decode(nn.Module):
    ...
    def forward(self, x, direction):
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
            # Force the latent vector to half precision and decode within autocast
            x_decoded = (_vae.decode((x / _vae.config.scaling_factor).half()).sample).clamp(-1, 1)
        return x_decoded
...
class VINE_Turbo(torch.nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, x, secret=None, timesteps=None):
        ...
        # Extract the raw model if it was automatically wrapped by Accelerate
        raw_vae_dec = self.vae_dec.module if hasattr(self.vae_dec, "module") else self.vae_dec

        # Execute using the raw layer inside our controlled fp16 block
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
            x_out_decoded = raw_vae_dec(x_out.half(), direction="a2b").to(x.dtype)
